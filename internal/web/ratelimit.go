...
const (
	...
	//
	// Counted per connection, and a tab holds one control stream plus one per
	// run it is watching, so the old cap of 32 was sixteen tabs only while a tab
	// watched a single thread. Refusing one costs the client a retry, which is
	// cheap now that a refusal cannot also spend its way into a rate limit.
	maxSockets = 64
