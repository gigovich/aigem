...
[latest release](https://github.com/gigovich/aigem/releases/latest).
...
**Your model or theirs.** A local llama.cpp server (aigem can install and manage
the daemon for you), an OpenAI API key, a ChatGPT subscription, or any
OpenAI-compatible endpoint you add. Token cost and remaining quota are tracked
from real responses, so you can compare burn rate across models.
...
is not sandboxed beyond its working directory - the deny lists catch catastrophic
commands, not every bad idea. Only approve `bash` calls you understand.
